  ...
  const handleDelete = async () => {
    try {
      ...
      const res = await axios.delete(
        `${API}/deleteChat/${convo._id}/${user?.id}`
      );
  ...
  const saveEditedTitle = async () => {
    ...
    try {
      const res = await axios.put(
        `${API}/updateChatTitle/${convo._id}/${user?.id}`,
        { title }
      );
      if (res.data.success) {
  ...
  return (
    <>
      <div
        ...
        className={`${
          showDropdown
            ? "fixed top-0 left-0 w-full h-full z-10 opacity-50"
            : "-z-10"
        }`}
      ></div>
      <li className="flex justify-between items-center relative">
        ...
        <div className="flex items-center gap-2 mr-2">
          ...
          {showDropdown && (
            <div className="absolute right-7 top-7 bg-[#212121]  rounded-md shadow-lg py-1 px-1 z-20 w-32">
              <button
                className="flex items-center cursor-pointer gap-2 hover:bg-white/20 rounded-md w-full px-3  py-2 text-sm text-left text-gray-200"
                onClick={handleEdit}
              >
