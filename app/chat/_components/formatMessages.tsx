...
const formatMessageContent = (message: Message, isMessageLoading: boolean) => {
    if (isMessageLoading && message.role === "assistant" && message.content === "") {
        return <span className="animate-pulse text-gray-400">AI is thinking...</span>;
    }

    return message.content.split(/```(\w+)?\n([\s\S]*?)```/g).map((part, index, arr) => {
        if (index % 3 === 2) {
...
const CodeBlock = ({ code, language }: { code: string; language: string }) => {
    ...
    return (
        <div className="relative">
            ...
            {/* Syntax Highlighting */}
            <SyntaxHighlighter language={language} style={dracula} className="rounded-lg">
                {code}
            </SyntaxHighlighter>
        </div>
    );
