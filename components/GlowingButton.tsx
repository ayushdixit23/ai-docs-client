...
const GlowingButton = ({ text = "Get Started" }: { text?: string | React.ReactNode }) => {
    return (
        <a href={"/chat"} target='_blank'>
            <motion.div whileHover={{ scale: 1.02 }} className="group cursor-pointer relative">
                ...
                <button className="relative flex justify-center items-center gap-1 cursor-pointer text-sm sm:text-base px-5 text-white/80 hover:text-white rounded-lg bg-black sm:px-6 py-2">
                    {text}
                </button>
            </motion.div>
        </a>
